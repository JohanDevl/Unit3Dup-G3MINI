...
def get_type_id(value):
    ...
    for gemini in gemini_data['TYPE_ID'].items():
        if gemini[1] == value:
            return gemini[0]

def get_category(value):
    # get key based on value
    for gemini in gemini_data['CATEGORY'].items():
        if gemini[1] == value:
            return gemini[0]

def get_tags(value):
    # get key based on value
    for gemini in gemini_data['TAGS'].items():
        if gemini[1] == value:
            print(gemini[0])
            return gemini[0]
...
def test_content_manager():
    ...
    for content in contents:
        ...
        db_online = DbOnline(media=content, category=content.category,no_title=cli_scan.notitle)
