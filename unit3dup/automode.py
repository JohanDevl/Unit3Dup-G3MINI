...
class Auto:
    ...
    def upload(self):
        ...
        if self.is_dir:
            if self.auto == "man":
                # -u command avec un dossier : traiter le dossier comme une release pack
                # Passer le dossier lui-même, pas les fichiers individuels
            ...
            # For a single file, folder should be the parent directory and subfolder should be the filename
            parent_dir = os.path.dirname(self.path)
            filename = os.path.basename(self.path)
            return self._lists(files_path=[], subfolders_path=[filename], folder=parent_dir)
    ...
    def _lists(self, files_path: list, subfolders_path: list, folder: str = None):

        # Use provided folder or default to self.path
        base_folder = folder if folder is not None else self.path
        
        results = []
        for media_path in files_path + subfolders_path:
            # Si media_path est un chemin absolu ou un dossier complet, utiliser directement
            if os.path.isabs(media_path) or os.path.isdir(media_path):
                # C'est un chemin complet de dossier, utiliser directement comme folder et subfolder vide
                result = Media(folder=media_path, subfolder="")
            else:
                # C'est un chemin relatif, utiliser base_folder comme folder
                result = Media(folder=base_folder, subfolder=media_path)
